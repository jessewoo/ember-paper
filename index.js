...
const materialComponents = require("material-components-web");
...
const componentDependencies = {
  ...
  "paper-input": {
    styles: ["components/input/input.scss", "components/input/input-theme.scss"]
  },
  "paper-item": {
    dependencies: [
      "paper-checkbox",
...
module.exports = {
  ...
  treeForStyles(tree) {
    ...
    let materialStyles = ["material-components-web.scss"];
    ...
    let materialScssFiles = new Funnel(
      this.pathBase("material-components-web"),
      {
        files: materialStyles,
        // srcDir: '/',
        destDir: "material-components",
        annotation: "MaterialCss"
      }
    );
    ...
    //crate file ember--paper-components.scss that only importss angular-material files
    ...
    let importer2 = writeFile(
      "ember-paper-components2.scss",
      materialStyles.map(
        path => `/*HELLO*/ @use './material-components/${path}';`
      )
      // md-sidenav{background:green !important;}`
    );
    let mergedTrees = new BroccoliMergeTrees(
      [materialScssFiles, importer2, angularScssFiles, importer, tree],
      { overwrite: true }
    );
